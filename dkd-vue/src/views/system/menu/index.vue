<template>
   <div class="app-container">
      ...
      >
         ...
         <el-table-column :label="$t('menu.operation')" align="center" width="210" class-name="small-padding fixed-width">
            <template #default="scope">
               <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:menu:edit']">{{ $t('menu.editMenu') }}</el-button>
               <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)" v-hasPermi="['system:menu:add']">{{ $t('menu.addMenu') }}</el-button>
               <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:menu:remove']">{{ $t('menu.deleteMenu') }}</el-button>
            </template>
         </el-table-column>
      </el-table>
      ...
      <el-dialog :title="title" v-model="open" width="680px" append-to-body>
         <el-form ref="menuRef" :model="form" :rules="rules" label-width="100px">
            <el-row>
               ...
               <el-col :span="12">
                  <el-form-item :label="$t('menu.displayName')" prop="menuName">
                     <el-input v-model="form.menuName" :placeholder="$t('menu.displayNamePlaceholder')" />
                  </el-form-item>
               </el-col>
               <el-col :span="12">
               ...
               <el-col :span="12" v-if="form.menuType != 'F'">
                  <el-form-item prop="path">
                     ...
                     <el-input v-model="form.path" :placeholder="$t('menu.routeAddressPlaceholder')" />
                  </el-form-item>
               </el-col>
               ...
               <el-col :span="12" v-if="form.menuType != 'M'">
                  <el-form-item>
                     <el-input v-model="form.perms" :placeholder="$t('menu.perms')" maxlength="100" />
                     <template #label>
                        <span>
               ...
               <el-col :span="12" v-if="form.menuType == 'C'">
                  <el-form-item>
                     ...
                     <el-radio-group v-model="form.isCache">
                        ...
                        <el-radio label="1">{{ $t('menu.no') }} {{ $t('menu.cache') }}</el-radio>
                     </el-radio-group>
                  </el-form-item>
               </el-col>
               <el-col :span="12" v-if="form.menuType != 'F'">
         ...
         <template #footer>
            <div class="dialog-footer">
               <el-button type="primary" @click="submitForm">{{ $t('menu.confirm') }}</el-button>
...
const data = reactive({
  ...
  rules: {
    menuName: [{ required: true, message: t('menu.menuName') + "不能为空", trigger: "blur" }],
    orderNum: [{ required: true, message: t('menu.orderNum') + "不能为空", trigger: "blur" }],
    path: [{ required: true, message: t('menu.routeAddress') + "不能为空", trigger: "blur" }]
  },
});
...
function handleAdd(row) {
  ...
  open.value = true;
  title.value = t('menu.addMenuTitle');
}
/** 展开/折叠操作 */
...
async function handleUpdate(row) {
  ...
  getMenu(row.menuId).then(response => {
    ...
    open.value = true;
    title.value = t('menu.editMenuTitle');
  });
}
/** 提交按钮 */
function submitForm() {
    ...
    if (valid) {
      if (form.value.menuId != undefined) {
        updateMenu(form.value).then(response => {
          proxy.$modal.msgSuccess(t('menu.updateSuccess'));
          open.value = false;
          getList();
        });
      } else {
